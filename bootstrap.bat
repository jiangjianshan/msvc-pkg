...
if "%errorlevel%" neq "0" (
  ...
  if not exist "ninja-win.zip" (
    curl -L https://github.com/ninja-build/ninja/releases/download/v1.12.1/ninja-win.zip -o ninja-win.zip || goto :end
  )
...
if "%errorlevel%" neq "0" (
  ...
  if "%ARCH%" == "x64" (
    if not exist "python-3.12.6-amd64.exe" (
      curl -L https://www.python.org/ftp/python/3.12.6/python-3.12.6-amd64.exe -o python-3.12.6-amd64.exe || (
        echo "Faill to download python-3.12.6-amd64.exe"
        goto :end
      )
    )
    start /wait python-3.12.6-amd64.exe InstallAllUsers=1 TargetDir=C:\Python312 PrependPath=1 Include_test=0 || goto :end
  ) else (
    if not exist "python-3.12.6.exe" (
      curl -L https://www.python.org/ftp/python/3.12.6/python-3.12.6.exe -o python-3.12.6.exe || (
        echo "Faill to download python-3.12.6.exe"
        goto :end
      )
    )
    start /wait python-3.12.6.exe InstallAllUsers=1 TargetDir=C:\Python312 PrependPath=1 Include_test=0 || goto :end
  )
)
...
if "%errorlevel%" neq "0" (
  ...
  if "%ARCH%" == "x64" (
    if not exist "cmake-3.30.3-windows-x86_64.msi" (
      curl -L https://github.com/Kitware/CMake/releases/download/v3.30.3/cmake-3.30.3-windows-x86_64.msi -o cmake-3.30.3-windows-x86_64.msi || (
        echo "Fail to download cmake-3.30.3-windows-x86_64.msi"
        goto :end
      )
    )
    start /wait msiexec -i cmake-3.30.3-windows-x86_64.msi || goto :end
  ) else (
    if not exist "cmake-3.30.3-windows-i368.msi" (
      curl -L https://github.com/Kitware/CMake/releases/download/v3.30.3/cmake-3.30.3-windows-i368.msi -o cmake-3.30.3-windows-i368.msi || (
        echo "Fail to download cmake-3.30.3-windows-i386.msi"
        goto :end
      )
    )
    start /wait msiexec -i cmake-3.30.3-windows-i368.msi || goto :end
  )
)
echo Done
exit /b 0
...
if "%errorlevel%" neq "0" (
  ...
  if "%ARCH%" == "x64" (
    if not exist "Git-!GIT_VERSION!-64-bit.exe" (
      curl -L https://github.com/git-for-windows/git/releases/download/v!GIT_VERSION!.windows.1/Git-!GIT_VERSION!-64-bit.exe -o Git-!GIT_VERSION!-64-bit.exe || (
        echo "Fail to download Git-!GIT_VERSION!-64-bit.exe"
        goto :end
      )
    )
    ...
    if not exist "Git-!GIT_VERSION!-32-bit.exe" (
      curl -L https://github.com/git-for-windows/git/releases/download/v!GIT_VERSION!.windows.1/Git-!GIT_VERSION!-32-bit.exe -o Git-!GIT_VERSION!-32-bit.exe || (
        echo "Fail to download Git-!GIT_VERSION!-32-bit.exe"
        goto :end
      )
    )
...
if exist "!GIT_ROOT!" (
  ...
  if not exist "!GIT_ROOT!\usr\bin\make.exe" (
    call :download_extract https://repo.msys2.org/msys/x86_64/make-4.4-1-x86_64.pkg.tar.zst !GIT_ROOT!
  )
...
if "%errorlevel%" neq "0" (
  ...
  if not exist "yq.exe" (
    if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
      curl -L https://github.com/mikefarah/yq/releases/download/v4.44.3/yq_windows_amd64.exe -o yq.exe || goto :end
    ) else (
      curl -L https://github.com/mikefarah/yq/releases/download/v4.44.3/yq_windows_386.exe -o yq.exe || goto :end
    )
  )
