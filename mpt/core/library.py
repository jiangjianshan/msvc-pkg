...
class LibraryManager:
    ...
    def add_library(lib: str):
        """
        Interactively creates config.yaml for a new library.

        Args:
            lib (str): Library name (must match directory name in ports/)
        """
        # Create port directory using pathlib
        ...
        LibraryManager._process_source_config(config, lib)
        ...
        LibraryManager._process_run(config)
    ...
    def remove_library(lib: str) -> bool:
        """
        Remove a library configuration and directory.
        ...
        """
        try:
            # Create port directory path using pathlib
            lib_dir = ROOT_DIR / 'ports' / lib
