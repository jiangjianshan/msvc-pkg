...
class BaseConfig:
    ...
    def _load_yaml_config(config_path: Path, config_name: str) -> Optional[Dict[str, Any]]:
        ...
        if not config_path.exists():
            Logger.error(f"Configuration file not found: [bold red]{config_path}[/bold red]")
            return None

        if not config_path.is_file():
            Logger.error(f"Configuration path is not a file: [bold red]{config_path}[/bold red]")
            return None
        try:
...
class PackageConfig(BaseConfig):
    ...
    def load_all() -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            Logger.debug(f"Found [bold cyan]{len(libs_dirs)}[/bold cyan] potential library directories")
            ...
            Logger.info(f"Loaded configurations for [bold green]{len(libs)}[/bold green] out of [bold cyan]{len(libs_dirs)}[/bold cyan] libraries")
            return libs
        except Exception as e:
            Logger.exception(
...
class RequirementsConfig(BaseConfig):
    ...
    def load() -> List[str]:
        ...
        if config:
            ...
            Logger.debug(f"Loaded [bold cyan]{len(deps)}[/bold cyan] dependencies from requirements.yaml")
            return deps

        Logger.warning("No dependencies found in requirements.yaml")
        return []
